...
class OxygennaStacks
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_enqueue_scripts', array(&$this, 'enqueue_stack_css'));
    ...
    public function init()
    {
        ...
        $labels = array(
            'name'               => esc_html__('Stacks', 'lambda-admin-td'),
            'singular_name'      => esc_html__('Stack', 'lambda-admin-td'),
            'add_new'            => esc_html__('Add New', 'lambda-admin-td'),
            'add_new_item'       => esc_html__('Add New Stack', 'lambda-admin-td'),
            'edit_item'          => esc_html__('Edit Stack', 'lambda-admin-td'),
            'new_item'           => esc_html__('New Stack', 'lambda-admin-td'),
            'all_items'          => esc_html__('All Stacks', 'lambda-admin-td'),
            'view_item'          => esc_html__('View Stack', 'lambda-admin-td'),
            'search_items'       => esc_html__('Search Stack', 'lambda-admin-td'),
            'not_found'          => esc_html__('No Stack found', 'lambda-admin-td'),
            'not_found_in_trash' => esc_html__('No Stack found in Trash', 'lambda-admin-td'),
            'menu_name'          => esc_html__('Stacks', 'lambda-admin-td')
        );
    ...
    public function register_import_metabox()
    {
        ...
        add_meta_box('oxy-stack-imp-exp', esc_html__('Import / Export', 'lambda-admin-td'), array(&$this, 'import_metabox'), $this->post_type, 'advanced', 'low');
    }
        ...
        if ($access_type === 'direct') {
            ...
            if (!$wp_filesystem->is_dir($upload_dir)) {
                ...
                if (!$wp_filesystem->put_contents($index_filename, '', FS_CHMOD_FILE)) {
                    wp_die(esc_html__('Error Saving Index File', 'lambda-admin-td'));
                }
            }

            // everything should be set so lets save the CSS file.
            if (!$wp_filesystem->put_contents($filename, $css, FS_CHMOD_FILE)) {
                wp_die(esc_html__('Error Saving File', 'lambda-admin-td'));
            }
        }
    }
    ...
    public function enqueue_stack_css()
    {
        // create and enqueue the skin stack.
        $stack_id = apply_filters('oxy-stack-current-id', '');
        $upload_dir = wp_upload_dir();
        $css_folder_url = trailingslashit($upload_dir['baseurl']) . THEME_SHORT;
        $css_url = trailingslashit($css_folder_url) . 'stack-' . $stack_id . '.css';
        wp_enqueue_style(THEME_SHORT . '-theme-stack', set_url_scheme($css_url), array(THEME_SHORT.'-theme'));
    }
