...
class OxygennaOutConnectCheck extends OxygennaSystemCheck
{
    ...
    {
        $this->args = $args;
        parent::__construct(esc_html__('Outgoing HTTP Connections', 'lambda-admin-td'), 'warning');
    }
    ...
    public function check()
    {
        ...
        if ($this->ok) {
            $this->info = esc_html__('Your server can connect to the themes demo content data', 'lambda-admin-td');
            $this->value = $response['response']['code'] . ' - ' . $response['response']['message'];
        } else {
            $this->info = esc_html__('Your server can not connect to the themes demo content data', 'lambda-admin-td');
