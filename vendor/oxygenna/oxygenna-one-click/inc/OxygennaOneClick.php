...
class OxygennaOneClick
{
    ...
    public function import_start()
    {
        ...
        $return->data = esc_html__('Could not validate nonce', 'lambda-admin-td');
    ...
    public function import_posts()
    {
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'oxy-importer')) {
            ...
            if (is_array($posts) && count($posts) > 0) {
                ...
                $return->status = true;
                $return->data = esc_html__('Import Posts OK', 'lambda-admin-td');
            }
        }
    ...
    public function import_post($post)
    {
        ...
        $this->install_package->add_log_message(esc_html__('Starting import of "', 'lambda-admin-td') . $post['post_title'] . '"');
        ...
        if (!$this->post_already_installed($post)) {
            ...
            if ($post_return_id !== null) {
                ...
                $this->install_package->add_log_message(esc_html__('Post Created.', 'lambda-admin-td'));
            } else {
                $this->install_package->set_log_status(OXY_IMPORT_FAIL);
                $this->install_package->add_log_message(esc_html__('Failed to create post.', 'lambda-admin-td'));
            }
        } else {
            $this->install_package->set_log_status(OXY_IMPORT_EXISTS);
            $this->install_package->add_log_message(esc_html__('Import stopped reason - Post already exists.', 'lambda-admin-td'));
        }
    ...
    public function create_post($post)
    {
        ...
        if ($new_id !== 0) {
            ...
            $this->install_package->add_log_message(esc_html__('Import Post OK', 'lambda-admin-td'));
            ...
            if (isset($post['custom_fields'])) {
                ...
                $this->install_package->add_log_message(esc_html__('Added ' . count($post['custom_fields']) . ' custom fields', 'lambda-admin-td'));
            }
            ...
            if (isset($post['taxonomies'])) {
                ...
                $this->install_package->add_log_message(esc_html__('Added ' . count($taxonomies) . ' taxonomies', 'lambda-admin-td'));
            }

            // handle post_format
            if (isset($post['format']) && $post['format'] !== false) {
                $this->install_package->add_log_message(esc_html__('Set post format ' . $post['format'], 'lambda-admin-td'));
                set_post_format($new_id, $post['format']);
            }

            if (isset($post['attachments'])) {
                foreach ($post['attachments'] as $old_attachment_id) {
                    $this->install_package->add_log_message(esc_html__('Set attachment', 'lambda-admin-td'));
    ...
    public function create_attachment($post)
    {
        ...
        if (is_wp_error($response)) {
            $this->install_package->add_log_message(esc_html__('Could not download - ' . $image_url, 'lambda-admin-td'));
        } else {
            $image_body = wp_remote_retrieve_body($response);
            $this->install_package->add_log_message(esc_html__('Downloaded image data - ' . $image_url, 'lambda-admin-td'));
            ...
            if ($upload['error']) {
                $this->install_package->add_log_message(esc_html__('Could not upload to WordPress - ' . $upload['error'], 'lambda-admin-td'));
            }
            ...
            if (0 !== $attach_id) {
                ...
                $this->install_package->add_log_message(esc_html__('Attachment created', 'lambda-admin-td'));

                // regenerate thumbnails
                $attach_data = wp_generate_attachment_metadata($attach_id, $upload['file']);
                wp_update_attachment_metadata($attach_id, $attach_data);

                $this->install_package->add_log_message(esc_html__('Generated Thumbnails', 'lambda-admin-td'));
    ...
    public function import_menu()
    {
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'oxy-importer')) {
            ...
            $this->install_package->add_log_message(esc_html__('Started Import Menu.', 'lambda-admin-td'));
            ...
            if (false === $already_exists) {
                ...
                if (!is_wp_error($new_menu_id)) {
                    ...
                    $this->install_package->set_log_status(OXY_IMPORT_OK);
                    $this->install_package->add_log_message(esc_html__('Menu Created.', 'lambda-admin-td'));
                    ...
                    $this->install_package->set_log_status(OXY_IMPORT_FAIL);
                    $this->install_package->add_log_message(esc_html__('Call to wp_create_nav_menu failed.', 'lambda-admin-td'));
                }
            } else {
                $return->status = true;
                $return->data = 'Menu exists';
                $this->install_package->set_log_status(OXY_IMPORT_EXISTS);
                $this->install_package->add_log_message(esc_html__('Import stopped reason - Menu already exists.', 'lambda-admin-td'));
            }
    ...
    public function import_slideshow()
    {
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'oxy-importer')) {
            ...
            if (!$this->slideshow_already_installed($slideshow)) {
                        ...
                        switch($slideshow['type']) {
                            ...
                            case 'revslider':
                                if (class_exists('RevSliderSliderImport')) {
                                    ...
                                    $this->install_package->set_log_status(OXY_IMPORT_OK);
                                    $this->install_package->add_log_message(esc_html__('Revslider Created.', 'lambda-admin-td'));
                                    $return->status = true;
                                }
                                break;
                        }
                ...
                $this->install_package->set_log_status(OXY_IMPORT_EXISTS);
                $this->install_package->add_log_message(esc_html__('Import stopped reason - Slideshow already exists.', 'lambda-admin-td'));
            }
        }
