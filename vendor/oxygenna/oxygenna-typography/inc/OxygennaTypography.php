...
class OxygennaTypography
{
    ...
    { ?>
        <div class="error">
            <p><?php _e('Looks like you have the <strong>Oxygenna Typography Plugin</strong> Installed - this is no longer needed please delete it <a href="' . admin_url('plugins.php') . '">here on your plugins page</a>', 'lambda-admin-td'); ?></p>
        </div>
    <?php
    }
    ...
    public function google_fetch_fonts_list()
    {
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'google-fetch-fonts-nonce')) {
                ...
                $resp->message = __('Could not verify nonce', 'lambda-admin-td');
            }
        }
    ...
    public function google_update_font_list()
    {
        ...
        if ($ip !== $google_font_domain) {
            ...
            $resp->status = false;
            $resp->message = __('Could not resolve domain name googleapis.com - Please check your servers outgoing connections, DNS lookup & firewall settings', 'lambda-admin-td');
        }
    ...
    public function font_modal()
    {
        if (isset($_GET['nonce'])) {
            if (wp_verify_nonce($_GET['nonce'], 'font-modal')) {
                ...
                $save_button_text = $isNew ? __('Add font', 'lambda-admin-td') : __('Update Font', 'lambda-admin-td');
                ...
                $elements = array(
                    'body'       => __('All (body tag)', 'lambda-admin-td'),
                    'headings'   => __('Headings (h1-h6 tags)', 'lambda-admin-td'),
                    'forms'      => __('Forms (all input tags)', 'lambda-admin-td'),
                    'blockquote' => __('Block Quote (blockquote tag)', 'lambda-admin-td'),
                );
    ...
    public function fontstack_save()
    {
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'update-fontstack')) {
                ...
                // set response to ok
                $resp->status = true;
                $resp->message = __('Fontsack Updated Successfully', 'lambda-admin-td');
        ...
        if (isset($_POST['nonce'])) {
            if (wp_verify_nonce($_POST['nonce'], 'default-fonts')) {
                ...
                if (false !== $google_fonts) {
                    ...
                    $resp->message = __('Default Fonts Installed Successfully', 'lambda-admin-td');
                } else {
                    $resp->message = __('No Google Fonts Installed. Please go to ' . THEME_NAME . ' -> Fonts and update your google fonts', 'lambda-admin-td');
                }
            }
        }
