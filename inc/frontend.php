...
function oxy_create_social_options()
{
    ...
    foreach ($icons as $icon => $name) {
        $fields[] =  array(
            'name'    => sprintf(esc_html__('%s URL', 'lambda-admin-td'), $name),
            'id'      => sprintf(esc_html__('%s', 'lambda-admin-td'), $icon),
            'type'    => 'text',
            'default' => '',
            'attr'    =>  array(
                'class'    => 'widefat',
...
{
    $sizes['square-image'] = esc_html__('Square Image', 'lambda-admin-td');
    return $sizes;
}
...
register_nav_menus(array(
    'primary' => esc_html__('Primary Navigation', 'lambda-admin-td')
));
...
function oxy_load_scripts()
{
    ...
    // Loading flexslider separately so that woocommerce, when enabled, overrides it
    wp_enqueue_script('flexslider', OXY_THEME_URI . 'assets/js/flexslider.min.js', array('jquery'), false, true);
    // load js
    ...
    wp_localize_script(THEME_SHORT.'-theme', 'oxyThemeData', array(
        ...
        'scrollFinishedMessage'  => esc_html__('No more items to load.', 'lambda-td'),
        'hoverMenu'              => array(
...
function oxy_comment_callback($comment, $args, $depth)
{
    ...
    $tag = $depth === 1 ? 'li' : 'div'; ?>
    <<?php echo esc_html($tag) ?> <?php comment_class('media media-comment'); ?>>
                ...
                <div id="comment-<?php comment_ID(); ?>">
                    <h4 class="media-heading clearfix">
                        ...
                        <strong class="comment-reply pull-right">
                            <?php comment_reply_link(array_merge($args, array('reply_text' => esc_html__('reply', 'lambda-td'), 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
                        </strong>
                    </h4>
                    <?php comment_text(); ?>
                </div>
...
function oxy_filter_comment_form_defaults($defaults)
{
    ...
    $defaults['fields']['author'] = '<div class="row"><div class="form-group col-md-4"><label for="author">' . esc_html__('Your name *', 'lambda-td') . '</label><input id="author" name="author" type="text" class="input-block-level form-control" value="' . esc_attr($commenter['comment_author']) .  '"/></div>';
    $defaults['fields']['email']  = '<div class="form-group col-md-4"><label for="email">' . esc_html__('Your email *', 'lambda-td') . '</label><input id="email" name="email" type="text" class="input-block-level form-control" value="' . esc_attr($commenter['comment_author_email']) . '" /></div>';
    $defaults['fields']['url'] = '<div class="form-group col-md-4"><label for="url">' . esc_html__('Website', 'lambda-td') . '</label><input id="url" name="url" type="text" class="input-block-level form-control" value="' . esc_attr($commenter['comment_author_url']) . '" /></div></div>';


    $defaults['comment_field'] = '<div class="row"><div class="form-group col-md-12"><label for="comment">' . esc_html__('Your message', 'lambda-td') . '</label><textarea id="comment" name="comment" class="input-block-level form-control" rows="5"></textarea></div></div>';
...
function oxy_read_more_link($echo = true)
{
    ...
    $output = '';
    if (isset($post)) {
        $more_text = oxy_get_option('blog_readmore')? oxy_get_option('blog_readmore'): esc_html__('Read more', 'lambda-td');
...
function oxy_custom_search_form($form)
{
    ...
    $output .= '<input type="text" value="' . get_search_query() . '" name="s" id="s" class="form-control" placeholder="' . esc_html__('Search', 'lambda-td') . '"/>';
...
function oxy_wp_link_pages($args = '')
{
    $defaults = array(
        ...
        'nextpagelink' => esc_html__('Next page', 'lambda-td'),
        'previouspagelink' => esc_html__('Previous page', 'lambda-td'),
        'pagelink' => '%',
        'echo' => 1
    );
...
function oxy_filter_body_class($classes)
{
    ...
    if (is_page() || $is_blog || $is_portfolio || $is_staff  || $is_service || $wp_query->is_404 && !is_null($post)) {
        // if we are on the blog page make sure you use the blog page id for transparancy option
        ...
        if (get_post_meta($page_id, THEME_SHORT . '_site_top_nav_transparency', true) === 'on') {
            ...
            if ($oxy_theme->get_option('header_top_bar') === 'on' && (is_active_sidebar('above-nav-left') || is_active_sidebar('above-nav-right'))) {
...
function oxy_query_is_blog()
{
    ...
    return (is_object($wp_query) && (($wp_query->is_archive) || ($wp_query->is_author) || ($wp_query->is_category) || ($wp_query->is_home) || ($wp_query->is_single) || ($wp_query->is_tag)) && is_object($post) && ($post->post_type === 'post'));
}
...
/**
 * Filters document_title_separator set a separator.
 *
 * @param string $sep Optional separator.
 */
function oxy_set_title_separator($sep) {
    return '|';
}
add_filter('document_title_separator', 'oxy_set_title_separator');

/**
 * Filters document_title_parts to print a neat <title> tag based on what is being viewed.
 *
 * @param string $title Default title text for current view.
 * @param string $sep Optional separator.
 * @return string The filtered title.
 */
function oxy_set_wp_title($title)
{
...
add_filter('document_title_parts', 'oxy_set_wp_title', 10, 1);
...
function oxy_remove_title_from_icon_only_menu_item($title, $id = null) {
    if (null !== $id && is_nav_menu_item($id) && $title === 'ICON_ONLY') {
        return false;
    }
    return $title;
}
