...
if (class_exists('WPBakeryVisualComposerAbstract')) {
    ...
    function oxy_init_vc_codes()
    {
        ...
        vc_map(array(
            ...
            'category'                => esc_html__('Content', 'lambda-admin-td'),
        ...
        vc_map(array(
            ...
            'category'                => esc_html__('Bootstrap', 'lambda-admin-td'),
            ...
            ,
            'default_content' => '
            [vc_tab title="' . esc_html__('Tab 1', 'lambda-admin-td') . '" tab_id="' . $tab_id_1 . '"][/vc_tab]
            [vc_tab title="' . esc_html__('Tab 2', 'lambda-admin-td') . '" tab_id="' . $tab_id_2 . '"][/vc_tab]
            ',
            'js_view' => 'VcTabsView'
        ));
        ...
        vc_map(array(
            ...
            'name' => esc_html__('Video Player', 'lambda-admin-td'),
            'base'        => 'vc_video',
        ...
        vc_map(array(
            ...
            <button class="add_tab" title="'.esc_html__('Add accordion section', 'lambda-admin-td') . '">' . esc_html__('Add accordion section', 'lambda-admin-td') . '</button>
            </div>
            ',
            'default_content' => '
            [vc_accordion_tab title="' . esc_html__('Section 1', 'lambda-admin-td') . '" state="closed"][/vc_accordion_tab]
            [vc_accordion_tab title="' . esc_html__('Section 2', 'lambda-admin-td') . '" state="closed"][/vc_accordion_tab]
            ',
            'js_view' => 'VcAccordionView'
        ));

        vc_map(array(
            'name' => esc_html__('Accordion Section', 'lambda-admin-td'),
            ...
            'params' => array(
                array(
                    'type' => 'textfield',
                    'heading' => esc_html__('Title', 'lambda-admin-td'),
                    'param_name' => 'title',
                    'description' => esc_html__('Accordion section title.', 'lambda-admin-td')
               ),
                array(
                    'type' => 'dropdown',
                    'heading' => esc_html__('State', 'lambda-admin-td'),
                    'param_name' => 'state',
                    'value' => array(
                        esc_html__('Closed', 'lambda-admin-td') => 'closed',
                        esc_html__('Open', 'lambda-admin-td')   => 'open',
                   ),
                    'description' => esc_html__('Is this accordion panel open?', 'lambda-admin-td')
               ),
           ),
        ...
        vc_map(array(
            ...
            'icon'                    => 'icon-wpb-graph',
            'show_settings_on_create' => true,
            'category'                => esc_html__('Theme Elements', 'lambda-admin-td'),
        ...
        vc_map(array(
            ...
            'base'     => 'icon',
            'icon'     => 'icon-oxy-icon',
            'category' => esc_html__('Typography', 'lambda-admin-td'),
        ...
        if (class_exists('WP_Charts_Widget')) {
            vc_map(array(
        ...
        if (class_exists('WooCommerce')) {
            vc_map(array(
                ...
                'category'        => esc_html__('Woocommerce', 'lambda-admin-td'),
            ));
        ...
        if (class_exists('LS_Sliders')) {
            ...
            vc_map(array(
                'base' => 'layerslider_vc',
                'name' => esc_html__('Layer Slider', 'lambda-admin-td'),
                'icon' => 'icon-wpb-layerslider',
                'category' => esc_html__('Content', 'lambda-admin-td'),
                'description' => esc_html__('Place LayerSlider', 'lambda-admin-td'),
                'params' => array(
                    array(
                        'type' => 'dropdown',
                        'heading' => esc_html__('LayerSlider ID', 'lambda-admin-td'),
                        'param_name' => 'id',
                        'admin_label' => true,
                        'value' => $layer_sliders_ids,
                        'description' => esc_html__('Select your LayerSlider.', 'lambda-admin-td')
                   ),
                    array(
                        'type' => 'dropdown',
                        'heading' => esc_html__('LayerSlider by Slug', 'lambda-admin-td'),
                        'param_name' => 'slug',
                        'value' => $layer_sliders_slugs,
                        'description' => esc_html__('Select your LayerSlider using its optional slug (overrides id above).', 'lambda-admin-td')
                   ),
                    array(
                        'type' => 'textfield',
                        'heading' => esc_html__('Extra class name', 'lambda-admin-td'),
                        'param_name' => 'el_class',
                        'description' => esc_html__('If you wish to style particular content element differently, then use this field to add a class name and then refer to it in your css file.', 'lambda-admin-td')
                    )
                )
            ));
    ...
    function oxy_remove_vc_codes()
    {
        ...
        // Removes vc_section elements added as of VC 5.0
