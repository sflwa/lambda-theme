...
{
    return array(
        'name'               => esc_html__('Skins', 'lambda-admin-td'),
        'singular_name'      => esc_html__('Skin', 'lambda-admin-td'),
        'add_new'            => esc_html__('Add New', 'lambda-admin-td'),
        'add_new_item'       => esc_html__('Add New Skin', 'lambda-admin-td'),
        'edit_item'          => esc_html__('Edit Skin', 'lambda-admin-td'),
        'new_item'           => esc_html__('New Skin', 'lambda-admin-td'),
        'all_items'          => esc_html__('All Skins', 'lambda-admin-td'),
        'view_item'          => esc_html__('View Skin', 'lambda-admin-td'),
        'search_items'       => esc_html__('Search Skin', 'lambda-admin-td'),
        'not_found'          => esc_html__('No Skin found', 'lambda-admin-td'),
        'not_found_in_trash' => esc_html__('No Skin found in Trash', 'lambda-admin-td'),
        'menu_name'          => esc_html__('Skins', 'lambda-admin-td')
    );
}
...
function oxy_edit_columns_skins($columns)
{
    $columns = array(
        ...
        'activate'             => esc_html__('Activated', 'lambda-admin-td'),
...
function oxy_theme_install_mega_menu()
{
    ...
    if (wp_verify_nonce($_POST['nonce'], THEME_SHORT . '-oxy-mega-menu')) {
        ...
        if (count($menus) === 0) {
            $columns = array(
                'col-md-3'  => esc_html__('One Quarter Column (1/4)', 'lambda-admin-td'),
                'col-md-4'  => esc_html__('One Third Column (1/3)', 'lambda-admin-td'),
            );
...
/**
 * Fix for backend stack js error of VC using media without enqueuing it.
 *
...
function oxy_admin_fix_media_vc_stack_error($hook)
{
    ...
    if ('oxy_stack' === $screen->post_type && 'post' === $screen->base) {
        wp_enqueue_media();
    }
}
